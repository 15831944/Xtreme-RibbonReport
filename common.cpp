...
CString g_DataBase;
CString g_TablePrix;
CString g_CurDirectory;
CString g_RootDirectory;
...
{
	CString str,prec;
	prec.Format(_T("%d"),nPrec);
	prec ? prec = _T("%.")+prec+_T("f") : prec = _T("%")+prec+_T("d");
...
CXTPString RegGetString(HKEY hKey, const xtpchar* subKey, const xtpchar* name)
{
	HKEY hSubKey;
	DWORD type;
...
int RegPutString(HKEY hKey, const xtpchar* subKey, const xtpchar* name, const xtpchar* val)
{
	HKEY hSubKey;
	DWORD disp;
	int retval = 0;
...
int RegGetInt(HKEY hKey, const xtpchar* subKey, const xtpchar* name)
{
	HKEY hSubKey;
	DWORD type;
	DWORD val;
...
int RegPutInt(HKEY hKey, const xtpchar* subKey, const xtpchar* name, int val)
{
	HKEY hSubKey;
	DWORD disp;
	int retval = 0;
...
BOOL DataBackup(CString data, CString path)
{
	...
	cmd.SetType(CXTPADOCommand::adoCmdStoredProc);
	cmd.MakeInParam(_T("@backup_db_name"), adoVarWChar, 128, _variant_t(data));
	cmd.MakeInParam(_T("@filename"), adoVarWChar, 255, _variant_t(path));
	cmd.MakeOutParam(_T("@ret"), adoTinyInt, 1, ret);
	cmd.MakeOutParam(_T("@flag"), adoVarChar, 20, flag);
	cmd.Execute();
...
BOOL DataRestore(CString data, CString path)
{
	...
	CXTPADOCommand cmd(theCon, sql);
	cmd.SetType(CXTPADOCommand::adoCmdStoredProc);
	cmd.MakeInParam(_T("@restore_db_name"), adoVarWChar, 128, _variant_t(data));
	cmd.MakeInParam(_T("@filename"), adoVarWChar, 255, _variant_t(path));
	cmd.MakeOutParam(_T("@ret"), adoTinyInt, 1, ret);
	cmd.MakeOutParam(_T("@flag"), adoVarChar, 20, flag);
	cmd.Execute();
...
int Login()
{
	...
	CXTPADOCommand cmd(theCon, sql);
	cmd.MakeInParam(_T("sysuser"), adoVarChar, 20, _variant_t(g_SysUser));
	cmd.MakeInParam(_T("password"), adoVarChar, 20, _variant_t(g_SysPwd));
...
int SetSysStatus(bool remberpwd, bool autologin, bool advanced, bool stat)
{
	CString sql;
	sql.Format(_T("update %s[tb_users] set remberpwd = %d, autologin = %d, advanced = %d, state=%d where id = ?"), g_TablePrix,remberpwd,autologin,advanced,stat);
	CXTPADOCommand cmd(theCon, sql);
	cmd.MakeInParam(_T("id"), adoVarChar, 5, _variant_t(g_SysUid));
	//cmd.MakeInParam(_T("remberpwd"), adoTinyInt, 1, _variant_t(remberpwd));
	//cmd.MakeInParam(_T("autologin"), adoTinyInt, 1, _variant_t(autologin));
	//cmd.MakeInParam(_T("advanced"), adoTinyInt, 1, _variant_t(advanced));
	return cmd.Execute();
}
...
int AddSysUser()
{
	...
	cmd.MakeInParam(_T("md5pwd"), adoVarChar, 32, _variant_t(md5pwd));
	return cmd.Execute();
}
...
// 获取数据信息
XTPREPORTMSADODB::_RecordsetPtr GetRecordset(CString table, CString sql)
{
	...
	if(sql.IsEmpty())
		...
		sql.Format(_T("select * from %s[%s] where %s"), g_TablePrix, table, sql);
...
COleVariant GetAutoCode(CString table)
{
	...
	CXTPADOCommand cmd(theCon, sql);
	cmd.SetType(CXTPADOCommand::adoCmdStoredProc);
	cmd.MakeInParam(_T("@tbl"), adoVarWChar, 20, _variant_t(table));
	cmd.MakeOutParam(_T("@fid"), adoVarWChar, 20, fid);
	cmd.MakeOutParam(_T("@val"), adoVarChar, 10, val);
	cmd.Execute();
...
CString GetFieldName(CString table, int idx)
{
	...
	cmd.SetType(CXTPADOCommand::adoCmdStoredProc);
	cmd.MakeInParam(_T("@tbl"), adoVarWChar, 20, _variant_t(table));
	cmd.MakeInParam(_T("@idx"), adoInteger, 4, _variant_t(idx));
	cmd.MakeOutParam(_T("@fid"), adoVarWChar, 20, fid);
	cmd.Execute();
...
long CountField(CString table, CString field, CString cond1, CString cond2)
{
	...
	cmd.SetType(CXTPADOCommand::adoCmdStoredProc);
	cmd.MakeInParam(_T("@tbl"), adoVarWChar, 20, _variant_t(table));
	cmd.MakeInParam(_T("@fid"), adoVarWChar, 20, _variant_t(field));
	cmd.MakeInParam(_T("@cond1"), adoVarWChar, 20, _variant_t(cond1));
	cmd.MakeInParam(_T("@cond2"), adoVarWChar, 20, _variant_t(cond2));
	cmd.MakeOutParam(_T("@val"), adoBigInt, 8, val);
	cmd.Execute();
...
double SumTable(CString table, int idx)
{
	...
	cmd.SetType(CXTPADOCommand::adoCmdStoredProc);
	cmd.MakeInParam(_T("@tbl"), adoVarWChar, 20, _variant_t(table));
	cmd.MakeInParam(_T("@fid"), adoVarWChar, 20, _variant_t(fid));
	cmd.MakeOutParam(_T("@val"), adoDouble, 8, val);
	cmd.Execute();
...
void ResetCode(CString table)
{
	...
	rst.Open(sql,CXTPADORecordset::adoRstStoredProc);
	CString val;
...
BOOL CopyTable(CString tblOld, CString tblNew)
{
	...
	CXTPADOCommand cmd(theCon, sql);
	cmd.SetType(CXTPADOCommand::adoCmdStoredProc);
	cmd.MakeInParam(_T("@tblOld"), adoVarWChar, 30, _variant_t(tblOld));
	cmd.MakeInParam(_T("@tblNew"), adoVarWChar, 30, _variant_t(tblNew));
	cmd.MakeOutParam(_T("@ret"), adoTinyInt, 1, ret);
	cmd.Execute();
...
{
	SetConfigW((LPTSTR)(LPCTSTR)config);
}

void LogDebug(CString debug)
{
	LogDebugW((LPTSTR)(LPCTSTR)debug);
}
...
void LogDebugExp(CString debug, CException *e)
{
	...
	LogDebugExpW((LPTSTR)(LPCTSTR)debug, szError);
}

void LogInfo(CString info)
{
	LogInfoW((LPTSTR)(LPCTSTR)info);
}
...
void LogInfoExp(CString info, CException *e)
{
	TCHAR szError[1024];
	e->GetErrorMessage(szError,1024);
	LogInfoExpW((LPTSTR)(LPCTSTR)info, szError);
}

void LogWarn(CString warn)
{
	LogWarnW((LPTSTR)(LPCTSTR)warn);
}
...
void LogWarnExp(CString warn, CException *e)
{
	TCHAR szError[1024];
	e->GetErrorMessage(szError,1024);
	LogWarnExpW((LPTSTR)(LPCTSTR)warn, szError);
}

void LogError(CString error)
{
	LogErrorW((LPTSTR)(LPCTSTR)error);
}
...
void LogErrorExp(CString error, CException *e)
{
	TCHAR szError[1024];
	e->GetErrorMessage(szError,1024);
	LogErrorExpW((LPTSTR)(LPCTSTR)error, szError);
}

void LogFatal(CString fatal)
{
	LogFatalW((LPTSTR)(LPCTSTR)fatal);
}
...
void LogFatalExp(CString fatal, CException *e)
{
	TCHAR szError[1024];
	e->GetErrorMessage(szError,1024);
	LogFatalExpW((LPTSTR)(LPCTSTR)fatal, szError);
}