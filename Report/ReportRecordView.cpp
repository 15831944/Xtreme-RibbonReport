...
void CReportRecordView::OnInitialUpdate()
{
	...
	if (m_wndFilterEdit.GetSafeHwnd() == NULL)
	{
		...
		m_wndFilterEdit.SetHint(_T("输入过滤文字"));
		...
		for(int i=0; i<iRows; i++)
		{
			...
			if(pRecord != NULL)
			{
				...
				if(pItem != NULL)
				{
					...
					pItem->SetCaption(bCheck ? _T("是"):_T("否"));
				}
			}
		}
	}
...
void CReportRecordView::OnReportHyperlink(NMHDR * pNMHDR, LRESULT * /*result*/)
{
	...
	if (pItemNotify->nHyperlink >= 0)
		if(m_vecRecordItemCtrlData.size()) ShowReportPerfomance((CXTPReportRecordItemVariant*&)pItemNotify->pItem);
}
...
void CReportRecordView::OnReportHeaderRClick(NMHDR* pNMHDR, LRESULT* /*result*/)
{
	...
	menu.AppendMenu(MF_STRING, ID_REPORT_SORT_ASC, LoadResourceString(ID_REPORT_SORT_ASC));
	menu.AppendMenu(MF_STRING, ID_REPORT_SORT_DESC, LoadResourceString(ID_REPORT_SORT_DESC));
	menu.AppendMenu(MF_STRING, ID_REPORT_SORT_NO, LoadResourceString(ID_REPORT_SORT_NO));
	menu.AppendMenu(MF_SEPARATOR, (UINT)-1, (LPCTSTR)NULL);
	menu.AppendMenu(MF_STRING, ID_REPORT_GROUP_BYFIELD, LoadResourceString(ID_REPORT_GROUP_BYFIELD));
	menu.AppendMenu(MF_STRING, ID_REPORT_GROUP_SHOWBOX, LoadResourceString(ID_REPORT_GROUP_SHOWBOX));
	menu.AppendMenu(MF_STRING, ID_REPORT_COLUMN_AUTOUPDATESQL, LoadResourceString(ID_REPORT_COLUMN_AUTOUPDATESQL));
	menu.AppendMenu(MF_SEPARATOR, (UINT)-1, (LPCTSTR)NULL);
	menu.AppendMenu(MF_STRING, ID_REPORT_COLUMN_REMOVE, LoadResourceString(ID_REPORT_COLUMN_REMOVE));
	menu.AppendMenu(MF_STRING, ID_REPORT_COLUMN_FILTEREDIT, LoadResourceString(ID_REPORT_COLUMN_FILTEREDIT));
	menu.AppendMenu(MF_STRING, ID_REPORT_COLUMN_SUBLIST, LoadResourceString(ID_REPORT_COLUMN_SUBLIST));
	menu.AppendMenu(MF_SEPARATOR, (UINT)-1, (LPCTSTR)NULL);
	menu.AppendMenu(MF_STRING, ID_REPORT_COLUMN_AUTOSIZING, LoadResourceString(ID_REPORT_COLUMN_AUTOSIZING));
	menu.AppendMenu(MF_STRING, ID_REPORT_COLUMN_AUTOBESTFIT, LoadResourceString(ID_REPORT_COLUMN_AUTOBESTFIT));
	menu.AppendMenu(MF_STRING, ID_REPORT_COLUMN_FREEZE, LoadResourceString(ID_REPORT_COLUMN_FREEZE));
	menu.AppendMenu(MF_STRING, ID_REPORT_COLUMN_ROWNUMBER, LoadResourceString(ID_REPORT_COLUMN_ROWNUMBER));

	if (wndReport.IsGroupByVisible())
		menu.CheckMenuItem(ID_REPORT_GROUP_SHOWBOX, MF_BYCOMMAND|MF_CHECKED);

	if (wndReport.GetReportHeader()->IsShowItemsInGroups())
		menu.EnableMenuItem(ID_REPORT_GROUP_BYFIELD, MF_BYCOMMAND|MF_DISABLED);
	...
	for (nColumn = 0; nColumn < nColumnCount; nColumn++)
	{
		...
		if (pCol && pCol->IsVisible())
		{
			...
			if (!sCaption.IsEmpty())
				menuArrange.AppendMenu(MF_STRING, ID_REPORT_COLUMN_ARRANGEBY + nColumn, sCaption);
		}
	}

	menuArrange.AppendMenu(MF_SEPARATOR, 60, (LPCTSTR)NULL);
	menuArrange.AppendMenu(MF_STRING, ID_REPORT_COLUMN_ARRANGEBY + nColumnCount,
		LoadResourceString(ID_REPORT_COLUMN_SHOWINGROUPS));
	menuArrange.CheckMenuItem(ID_REPORT_COLUMN_ARRANGEBY + nColumnCount,
		MF_BYCOMMAND | ((wndReport.GetReportHeader()->IsShowItemsInGroups()) ? MF_CHECKED : MF_UNCHECKED)  );
	menu.InsertMenu(0, MF_BYPOSITION | MF_POPUP, (UINT_PTR) menuArrange.m_hMenu,
		LoadResourceString(ID_REPORT_COLUMN_ARRANGEBY));
	...
	menuAlign.InsertMenu(1, MF_BYPOSITION | MF_POPUP, (UINT_PTR) menuAlignText.m_hMenu,_T("文本"));

	////  create Icon alignment menu
	CMenu menuAlignIcon;
	if(!menuAlignIcon.CreatePopupMenu()) return;

	menuAlignIcon.AppendMenu(MF_STRING, ID_REPORT_COLUMN_ICON_ALIGNMENT_LEFT,
		LoadResourceString(ID_REPORT_COLUMN_ICON_ALIGNMENT_LEFT));
	menuAlignIcon.AppendMenu(MF_STRING, ID_REPORT_COLUMN_ICON_ALIGNMENT_RIGHT,
		LoadResourceString(ID_REPORT_COLUMN_ICON_ALIGNMENT_RIGHT));
	menuAlignIcon.AppendMenu(MF_STRING, ID_REPORT_COLUMN_ICON_ALIGNMENT_CENTER,
		LoadResourceString(ID_REPORT_COLUMN_ICON_ALIGNMENT_CENTER));
	...
	menuAlign.InsertMenu(2, MF_BYPOSITION | MF_POPUP, (UINT_PTR) menuAlignIcon.m_hMenu, _T("图标"));

	///	
	menu.InsertMenu(11, MF_BYPOSITION | MF_POPUP, (UINT_PTR) menuAlign.m_hMenu,
		LoadResourceString(ID_REPORT_COLUMN_ALIGNMENT));
	...
	if (nMenuResult >= ID_REPORT_COLUMN_ARRANGEBY && nMenuResult < ID_REPORT_COLUMN_ALIGNMENT)
	{
		...
		// group by item
		if (ID_REPORT_COLUMN_ARRANGEBY + nColumnCount == nMenuResult)
...
void CReportRecordView::OnReportChecked(NMHDR*  pNotifyStruct, LRESULT* /*result*/)
{
	...
	if(pSelRow->GetType() != xtpRowTypeHeader)
	{
		CXTPReportRecord* pSelItemRcd = pSelItem->GetRecord();
		if(!pSelItemRcd) return;
		pSelItem->SetCaption(pSelItem->IsChecked() ? _T("是"):_T("否"));
		CString strTable = GetCurTable();
		CString field = GetFieldName(strTable,pSelItem->GetIndex()+1);
		CString code = ((CXTPReportRecordItemVariant*)pSelItemRcd->GetItem(0))->GetValue().bstrVal;
...
void CReportRecordView::OnReportItemRClick(NMHDR* pNMHDR, LRESULT* /*result*/)
{
	...
	{
		menu.AppendMenu(MF_STRING, ID_REPORT_GROUP_EXPAND, LoadResourceString(ID_REPORT_GROUP_EXPAND));
		menu.AppendMenu(MF_STRING, ID_REPORT_GROUP_COLLAPSE, LoadResourceString(ID_REPORT_GROUP_COLLAPSE));
	} 
	else
	{
	...
	switch (nMenuResult)
	{
	case ID_REPORT_GROUP_EXPAND:
		wndReport.ExpandAll(TRUE);
		break;
	case ID_REPORT_GROUP_COLLAPSE:
...
{
	TCHAR szFilter[] = _T("Access2007文件|*.accdb|Access2003文件|*.mdb|所有文件|*.*||");
	CFileDialog fd(TRUE, _T("accdb"), NULL, 0, szFilter, this);
	if(IDOK == fd.DoModal())
	{
		CXTPADOConnection con;
		con.SetDataDriver(CXTPADOConnection::adoACEAccess);
		con.SetDataBase(fd.GetPathName());
		con.InitConnectionString();
		if (con.Open())
		{
...
void CReportRecordView::OnToolsImport()
{
	...
	if(IDOK == FileDlg.DoModal())
	{
		...
		if(file.Open(FileDlg.GetPathName(), CFile::modeRead))
		{
			...
			file.Close();
			wndReport.Populate();
			msgInf(FileDlg.GetFileName() + " 文件导入成功!");
		}
	}
...
void CReportRecordView::DataBindRecordset(XTPREPORTMSADODB::_RecordsetPtr pRst, BOOL bInitRecordItemControlDataList)
{
	...
	CString strTable = GetCurTable();
	if(pRst != NULL)
		pDataManager->SetDataSource(pRst);
	else
		pDataManager->SetDataSource(GetRecordset(strTable));
	pDataManager->DataBind();
	
	if(bInitRecordItemControlDataList)
		InitRecordItemControlDataList();
...
CXTPReportControl& CReportRecordView::ShowReportPerfomance(CXTPReportRecordItemVariant*& pItem, bool bHeader)
{
	...
	RecordItemCtrlData itemCtrlData;
	for(int i=0; i<m_vecRecordItemCtrlData.size(); i++)
	{
		if(m_vecRecordItemCtrlData[i].m_column == pItem->GetIndex() && m_vecRecordItemCtrlData[i].m_ctrl == XTP_REPORT_ITEM_CTRL_LINK)
		{
			itemCtrlData = m_vecRecordItemCtrlData[i];

			if (!itemCtrlData.m_table.IsEmpty() && pView != NULL)
			{
				pView->SetRecordItem(pItem);
				CXTPReportDataManager* pDataManager = pView->m_wndReport.GetDataManager();
				pDataManager->SetDataSource(GetRecordset(itemCtrlData.m_table));
				pDataManager->DataBind();
			}
		}
	}
...
CXTPReportControl& CReportRecordView::ShowReportPerfomance(int nColumn, bool bHeader)
{
	...
	for(int i=0; i<m_vecRecordItemCtrlData.size(); i++)
	{
		if(m_vecRecordItemCtrlData[i].m_column == nColumn && m_vecRecordItemCtrlData[i].m_column == XTP_REPORT_ITEM_CTRL_LINK)
		{
			itemCtrlData = m_vecRecordItemCtrlData[i];
...
RecordItemCtrlDataList& CReportRecordView::GetrRecordItemCtrlDataList()
{
	return m_vecRecordItemCtrlData;
}

void CReportRecordView::AddRecordItemCtrlData(RecordItemCtrlData& itemCtrlData)
{
	m_vecRecordItemCtrlData.push_back(itemCtrlData);
}

void CReportRecordView::DelRecordItemCtrlData(int index)
{
	m_vecRecordItemCtrlData.erase(m_vecRecordItemCtrlData.begin()+index);
}
...
void CReportRecordView::InitRecordItemControlDataList()
{
	...
	{
		RecordItemCtrlData itemCtrlData(i);
		if(m_nType == XTP_REPORT_TABLE_TYPE_TP) itemCtrlData.m_editable = FALSE;
		m_vecRecordItemCtrlData.push_back(itemCtrlData);
	}
}
...
void CReportRecordView::SetRecordItemControl()
{
	for(int i=0; i<m_vecRecordItemCtrlData.size(); i++)
	{
		SetRecordItemControl(m_vecRecordItemCtrlData[i]);
	}
}

void CReportRecordView::SetRecordItemControl(RecordItemCtrlData& itemCtrlData)
{
	m_vecRecordItemCtrlData[itemCtrlData.m_column] = itemCtrlData;

	CXTPReportControl& wndReport = GetReportCtrl();
	CXTPReportColumns *pColumns = m_wndReport.GetColumns();
	if(!pColumns) return;
	CXTPReportColumn *pColumn = pColumns->GetAt(itemCtrlData.m_column);
	if(!pColumn) return;
	CXTPReportRecords* pRecords = wndReport.GetRecords();
	if(!pRecords) return;
	int iRows = pRecords->GetCount();
	for(int i=0; i<iRows; i++)
	{
		...
		CXTPReportRecordItem* pItem = pRecord->GetItem(itemCtrlData.m_column);
		if(!pItem) return;
		int nSize = pItem->GetCaption().GetLength();
		pItem->SetItemData((DWORD_PTR)(itemCtrlData.m_ctrl));
		pItem->SetEditable(itemCtrlData.m_editable);
		switch(itemCtrlData.m_ctrl)
		{
		...
		case XTP_REPORT_ITEM_CTRL_COMBOBOX:
			{
				CString* ptr = (CString*)itemCtrlData.m_ptr;
				if(ptr != NULL)
				{
					CStringArray strs;
...
void CReportRecordView::SetRecordItemControl(RecordItemCtrlDataList& vecItemData)
{
	for(int i=0; i<vecItemData.size(); i++)
	{
		m_vecRecordItemCtrlData[vecItemData[i].m_column] = vecItemData[i];
		SetRecordItemControl(vecItemData[i]);
	}
}

void CReportRecordView::SetRecordItemControl(int column, DWORD ctrl, CString table, BOOL header, BOOL editable, DWORD_PTR ptr, DWORD alignment)
{
	m_vecRecordItemCtrlData[column] = RecordItemCtrlData(column,ctrl,table,header,editable,ptr,alignment);
	SetRecordItemControl(m_vecRecordItemCtrlData[column]);
}
...
void CReportRecordView::SetHeaderRecordItemControl()
{
	for(int i=0; i<m_vecRecordItemCtrlData.size(); i++)
	{
		SetHeaderRecordItemControl(m_vecRecordItemCtrlData[i]);
	}
}

void CReportRecordView::SetHeaderRecordItemControl(RecordItemCtrlData& itemCtrlData)
{
	m_vecRecordItemCtrlData[itemCtrlData.m_column] = itemCtrlData;
...
void CReportRecordView::SetHeaderRecordItemControl(RecordItemCtrlDataList& vecItemData)
{
	for(int i=0; i<vecItemData.size(); i++)
	{
		m_vecRecordItemCtrlData[vecItemData[i].m_column] = vecItemData[i];
...
void CReportRecordView::SetHeaderRecordItemControl(int column, DWORD ctrl, CString table, BOOL header, BOOL editable, DWORD_PTR ptr, DWORD alignment)
{
	m_vecRecordItemCtrlData[column] = RecordItemCtrlData(column,ctrl,table,header,editable,ptr,alignment);
	SetHeaderRecordItemControl(m_vecRecordItemCtrlData[column]);
}
...
void CReportRecordView::SetFooterRecordItemControl()
{
	for(int i=0; i<m_vecRecordItemCtrlData.size(); i++)
	{
		SetFooterRecordItemControl(m_vecRecordItemCtrlData[i]);
	}
}

void CReportRecordView::SetFooterRecordItemControl(RecordItemCtrlData& itemCtrlData)
{
	m_vecRecordItemCtrlData[itemCtrlData.m_column] = itemCtrlData;
...
void CReportRecordView::SetFooterRecordItemControl(RecordItemCtrlDataList& vecItemData)
{
	for(int i=0; i<vecItemData.size(); i++)
	{
		m_vecRecordItemCtrlData[vecItemData[i].m_column] = vecItemData[i];
...
void CReportRecordView::SetFooterRecordItemControl(int column, DWORD ctrl, CString table, BOOL header, BOOL editable, DWORD_PTR ptr, DWORD alignment)
{
	m_vecRecordItemCtrlData[column] = RecordItemCtrlData(column,ctrl,table,header,editable,ptr,alignment);
	SetFooterRecordItemControl(m_vecRecordItemCtrlData[column]);
}

void CReportRecordView::SetRecordItemEditable()
{
	if(!m_vecRecordItemCtrlData.size()) return;
	...
	for(int i=0; i<iRows; i++)
	{
		CXTPReportRecord* pRecord = pRecords->GetAt(i);
		if(!pRecord) return;
		for(int j=0; j<m_vecRecordItemCtrlData.size(); j++)
		{
			CXTPReportRecordItem* pItem = pRecord->GetItem(m_vecRecordItemCtrlData[j].m_column);
			if(!pItem) return;
			pItem->SetEditable(m_vecRecordItemCtrlData[j].m_editable);
		}
	}
}
...
{
	m_vecRecordItemCtrlData[nColumn].m_editable = bEditable;
	CXTPReportControl& wndReport = GetReportCtrl();
	CXTPReportRecords* pRecords = wndReport.GetRecords();
	if(!pRecords) return;
	int iRows = pRecords->GetCount();
...
void CReportRecordView::SetRecordItemEditable(xtp_vector<int> nColumns, BOOL bEditable)
{
	for(int i=0; i<nColumns.size(); i++)
		m_vecRecordItemCtrlData[nColumns[i]].m_editable = bEditable;
...
{
	if(m_vecRecordItemCtrlData.size())
		ShowReportPerfomance(m_vecRecordItemCtrlData[0].m_column);
}
